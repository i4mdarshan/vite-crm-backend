...
class ManageLeadContacts extends Model
{
    use HasFactory;


    //method to add contact
    public static function add_contact($lead_id,$employee_id,$request)
    {
        $contact_info = new ManageLeadContacts;
        $contact_info->employee_id = $employee_id;
        $contact_info->lead_id = $lead_id;
        ...
        $contact_info->contact_notes = $request->contact_notes;
        $contact_info->isActive = $request->contact_status;

        if(request()->hasFile('contact_image')){
            $name = $request->file('contact_image')->getClientOriginalName();
            $request->file('contact_image')->move(public_path().'/uploads/leads/contact-person-images/', $name);
    ...
    {
        // dd($request);
        $contact_info = ManageLeadContacts::findOrFail($contact_id);
        ...
        $contact_info->contact_notes = isset($request->contact_notes) ? $request->contact_notes : $contact_info->contact_notes;
        ...
        if(request()->hasFile('contact_image')){
            if($contact_info->contact_person_image){
                $imagePath = public_path().'/uploads/leads/contact-person-images/'.$contact_info->contact_person_image;
