{{-- <div style="page-break-after: auto;"> --}}
    ...
    <head>
        <title>{{ ucwords($quotation_details->client_name)."_".$quotation_details->quotation_no }}</title>
        <style type="text/css">
            body {
    ...
    <body>
        <div class="row">
            <div class="title">
                <h2 class="head">{{ str_replace('_',' ',strtoupper($quotation_details->quotation_type)) }}</h2>
            </div>
            <table class="border" style="height: 80%;">
                <tr>
                    <td width="50%">Firm <br><strong>{{ ucwords($quotation_details->firm_name) }}</strong></td>
                    <td width="10%" style=" text-align: center;"><strong>{{$quotation_details->quotation_no}}</strong></td>
                    <td width="16.5%" style=" text-align: center;">Dated</td>
                    <td width="23.5%" style=" text-align: center;"><strong>{{ date('j F, Y',strtotime($quotation_details->quotation_date)) }}</strong></td>
                </tr>
                <tr>
                    <td rowspan="3" width="50%">Address <br><strong>{!! nl2br(e($quotation_details->firm_address)) !!}</strong></td>
                    {{-- <td rowspan="3" width="50%">Address <br><strong>{!! nl2br(e(str_replace('__','&#13;',$quotation_details->firm_address))) !!}</strong></td> --}}
                    <td width="35%" style=" text-align: left;">Payment Type</td>
                    <td colspan="2" width="15%" style=" text-align: left;"><strong>{{str_replace('_',' ',strtoupper($quotation_details->payment_type))}}</strong></td>
                </tr>
                <tr>
                    <td width="35%" style=" text-align: left;">Sales Person Name</td>
                    <td colspan="2" width="15%" style=" text-align: left;"><strong>{{ ucwords($quotation_details->employee->full_name) }}</strong></td>
                </tr>
                <tr>
                    <td width="35%" style=" text-align: left;">Dispatch Thru</td>
                    <td colspan="2" width="15%" style=" text-align: left;"><strong>{{ strtoupper($quotation_details->transport) }}</strong></td>
                </tr>
                <tr>
                    <td width="50%">Client <br><strong>{{ ucwords(str_replace('_', ' ', $quotation_details->client_name)) }}</strong></td>
                    <td width="16.5%">Terms of Delivery </td>
                    <td colspan="2">
                        <strong>
                            {{ ucwords(str_replace('_',' ',$quotation_details->term_of_supply)) }}
                            <br>
                            {{$quotation_details->payment_condition." ".ucwords($quotation_details->payment_time)." ".strtoupper($quotation_details->payment_type)}}
                        </strong>
                    </td>
                </tr>
                <tr>
                    {{-- <td width="50%">Address <br><strong>{!! nl2br(e(str_replace('__','&#13;', $quotation_details->client_address))) !!}</strong></td> --}}
                    <td rowspan="3" width="50%">Address <br><strong>{{ ucwords(str_replace('_', ' ', $quotation_details->client_address)) }}</strong></td>
                    <td width="16.5%">Destination </td>
                    <td colspan="2"><strong>{{$quotation_details->booking_destination}}</strong></td>
                </tr>
                <tr>
                    <td width="35%" style=" text-align: left;">Dispatch Date</td>
                    <td colspan="2" width="15%" style=" text-align: left;"><strong>{{ date('j F, Y',strtotime($quotation_details->dispatch_date)) }}</strong></td>
                </tr>
                <tr>
                    <td width="35%" style=" text-align: left;">Dispatch Status</td>
                    <td colspan="2" width="15%" style=" text-align: left;"><strong>{{ ucwords($quotation_details->dispatch_status) }}</strong></td>
                </tr>
                <tr>
                    <td colspan="4" class="border" style="padding: 0px !important;">
                        <?php $count = 1; ?>
                        @foreach($quotation_details->particulars->chunk(17) as $chunk)
                        ...
                        <table class="border">
                            {{-- <tr>
                                ...
                                <td class="border-none" style="border-bottom: 1px solid #000; padding: 0px 0px 0px 10px !important; text-align: left; vertical-align: center; font-size: 50px !important;" width="18%">
                                    <strong>
                                        <span class="inr-sign">
                                            {{ number_format($quotation_details->quotation_sub_total,2) }}
                                        </span>
                                    </strong>
                                </td>
                            </tr> --}}
                            {{-- <tr>
                                ...
                                <td class="border-none" style="border-bottom: 1px solid #000; padding-top: 10px !important; text-align: left; vertical-align: center; font-size: 50px !important;" width="12.5%">
                                    <strong>
                                        Tax ({{ $quotation_details->quotation_tax_percent }}%)
                                    </strong>
                                </td>
                                <td class="border-none" style="border-bottom: 1px solid #000; padding: 0px 0px 0px 10px !important; text-align: left; vertical-align: center; font-size: 50px !important;" width="18%">
                                    <strong>
                                        <span class="inr-sign">
                                            {{ number_format($quotation_details->quotation_tax,2) }}
                                        </span>
                                    </strong>
                                </td>
                            </tr> --}}
                            <tr>
                                ...
                                <td class="border-none" style="border-bottom: 1px solid #000; padding: 0px 0px 0px 10px !important; text-align: left; vertical-align: center; font-size: 50px !important;" width="18%">
                                    <strong>
                                        <span class="inr-sign">
                                            {{ number_format($quotation_details->quotation_total,2) }}
                                        </span>
                                    </strong>
                                </td>
                            </tr>
                ...
                <tr>
                    <td colspan="4" class="border">
                        ...
                        <span> 
                            <strong>
                                {{ $quotation_details->remarks ? $quotation_details->remarks : 'NA' }}
                            </strong>
                        </span>
                    </td>
                </tr>
                    ...
                    <td colspan="2" class="border">
                        ...
                        <table class="border-0">
                            <tr>
                                ...
                                <td class="border-0">: <strong>{{ ucwords($quotation_details->firm_bank_name) }}</strong></td>
                            </tr>
                            <tr>
                                <td class="border-0">
                                    A/c No.
                                </td>
                                <td class="border-0">: <strong>{{ $quotation_details->firm_bank_account_no }}</strong></td>
                            </tr>
                            <tr>
                                <td class="border-0">
                                    Branch & IFSC Code
                                </td>
                                <td class="border-0">: <strong>{{ $quotation_details->firm_branch_name." & ".$quotation_details->firm_bank_ifsc }}</strong></td>
                            </tr>
                        </table>
                    </td>
                    <td colspan="2">
                        For {{ ucwords($quotation_details->firm_name) }} ({{ Carbon::createFromDate($quotation_details->quotation_date)->format('Y').'-'.Carbon::createFromDate($quotation_details->quotation_date)->addYear()->format('Y') }})
                        <br>
                        <br>
                        <br>
                        ...
                        <table class="border-0" style="width: 100%;">
                            <tr>
                                <td class="border-0" style="word-wrap: break-word; max-width: 1px;">
                                    <strong>{{ ucwords($quotation_details->employee->full_name) }}</strong>
                                </td>
                            </tr>
                            <tr>
                                <td class="border-0">
                                    <strong>{{ ucwords($quotation_details->employee->user_detail->personal_phone_no_1) }}</strong>
                                </td>

                            </tr>
                            <tr>
                                <td class="border-0"  style="word-wrap: break-word; max-width: 1px;">
                                    <strong>{{ $quotation_details->employee->email }}</strong>
                                </td>
                            </tr>
                        </table>
                    </td>
